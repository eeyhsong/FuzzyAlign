...
class StableDiffusionXL_lowlevel(StableDiffusionXLPipeline):
    ...
    def prepare_latents_img2img(
        ...
        if timestep is not None:
            # 按 scheduler timestep 注入噪声，避免分布失真导致黑图
            if isinstance(generator, list):
                noise = torch.cat(
                    [
                        randn_tensor(latents[0:1].shape, generator=g, device=device, dtype=dtype)
                        for g in generator
                    ],
                    dim=0,
                )
    ...
    def prepare_latents_latent2img(
        ...
        # low_level_latent 来自 VAE encode 后已乘过 scaling_factor，不能再次缩放

        if timestep is not None:
            # 按 scheduler timestep 注入噪声，避免 img2img 起点过噪或失配
            if isinstance(generator, list):
                noise = torch.cat(
                    [
                        randn_tensor(latents[0:1].shape, generator=g, device=device, dtype=dtype)
...
def generate_ip_adapter_embeds_latent2img(
    self,
    ...
    **kwargs,
    ...
    if low_level_image is not None or low_level_latent is not None:
    ...
    if ip_adapter_image is not None:
        ...
        if self.do_classifier_free_guidance:
            image_embeds = [torch.cat([negative_image_embeds.unsqueeze(1), image_embeds[0]], dim=0)]
        image_embeds = [img_embed.to(device) for img_embed in image_embeds]
    
    if ip_adapter_embeds is not None:
        ip_adapter_embeds = ip_adapter_embeds.to(device=device, dtype=prompt_embeds.dtype)
        if ip_adapter_embeds.ndim == 2:
            ip_adapter_embeds = ip_adapter_embeds.unsqueeze(1)
        image_embeds = [ip_adapter_embeds]
        if self.do_classifier_free_guidance:
            image_embeds = [torch.cat([torch.zeros_like(img_embed), img_embed]) for img_embed in image_embeds]
        image_embeds = [img_embed.to(device) for img_embed in image_embeds]
    ...
    if not output_type == "latent":
        # make sure the VAE is in float32 mode, as it may overflow in float16/bfloat16
        original_vae_dtype = self.vae.dtype
        needs_upcasting = self.vae.config.force_upcast and self.vae.dtype in (torch.float16, torch.bfloat16)
...
class Generator4Embeds_latent2img:
    def __init__(self, num_inference_steps=4, img2img_strength=0.5, device=None) -> None:
        ...
        pipe.to(device)
        pipe.generate_ip_adapter_embeds = generate_ip_adapter_embeds_latent2img.__get__(pipe)
        # load ip adapter
        #     path_adapter, 
    ...
    def generate(self, image_embeds, low_level_latent, prompt='', generator=None):
        image_embeds = image_embeds.to(self.device, dtype=self.dtype)
        low_level_latent = low_level_latent.to(self.device, dtype=self.dtype)
        pipe = self.pipe

        latent_h, latent_w = low_level_latent.shape[-2], low_level_latent.shape[-1]
        image_h = int(latent_h * pipe.vae_scale_factor)
        image_w = int(latent_w * pipe.vae_scale_factor)
...
if __name__ == "__main__":
    ...
    generator = Generator4Embeds_latent2img(num_inference_steps=4)
    ...
    test_vae_latents = torch.load(os.path.join(f'./data/SDXL_vae_features_test.pt'), weights_only=True)['img_features']
    ...
    print('The shape of image_embeds[0].unsqueeze(0) is:', image_embeds[0].unsqueeze(0).shape)
    print('The shape of test_vae_latents[0].unsqueeze(0) is:', test_vae_latents[0].unsqueeze(0).shape)
    
    image_embeds = image_embeds[1].unsqueeze(0)
    vae_latents = test_vae_latents[1].unsqueeze(0)
    prompt_embeds = prompt_embeds[1].unsqueeze(0)
    pooled_prompt_embeds = pooled_prompt_embeds[1].unsqueeze(0)

    image = generator.generate(image_embeds, vae_latents, prompt_embeds, pooled_prompt_embeds)
    image.save('./latent_generation.png')
